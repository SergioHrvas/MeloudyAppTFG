...
void main() {
      ...
      child: MultiProvider(
          ...
          child: Consumer<Auth>(
            builder: (ctx, auth, _) => MaterialApp(
              ...
              home: Scaffold(
                body: !auth.isAuth ? PantallaLogin() : PantallaLecciones(),
              ),
            ),
          )),
    ));
  ...
  testWidgets('El botón de Preguntas del dashboard funciona correctamente',
      (WidgetTester tester) async {
    nock('http://${IP.ip}:5000').get('/api/question/get-questions?auth=null').reply(
        200,
        json.decode(
  ...
  testWidgets('El botón de Usuarios del dashboard funciona correctamente',
      (WidgetTester tester) async {
    nock('http://${IP.ip}:5000').get('/api/user/get-users/?auth=null').reply(
        200,
        json.decode(
        ...
        providers: [
          ...
          ChangeNotifierProxyProvider<Auth, UsuarioPerfil>(
            ...
            userant,
                  ...
                  UsuarioPerfil(
                      auth.token, userant == null ? Usuario() : userant.item)),
        ],
        child: Consumer<Auth>(
          builder: (ctx, auth, _) => MaterialApp(
              title: 'Meloudy',
